...
def BPA_net_rev(contract,df_payout,loading,v,redux=0,other='std'):
    ###########################################################################
    ...
    df_payout=df_payout*v    
    ##premium
    if contract=="collar":
        call=(pd.read_csv("BPA_payment_collar.csv").iloc[:,1])*v #payment tied to index + loading premium and scaled with v 
        call_prem=(call.mean()*(1+0.6379174))
    elif contract=="ind_swap":
        call=(pd.read_csv("BPA_payment_swap.csv").iloc[:,1])*v #payment tied to index + loading premium and scaled with v 
        call_prem=(call.mean()*(1+0.667731))
        
    ins_prem=(df_payout.mean()*(1+loading))*v ##priced using wang transform
    ...
    df_resources=pd.read_excel('net_rev_data.xlsx',sheet_name=1,skiprows=[0,1], usecols=[9])
    Nuc_y=df_resources.loc[[7]]
    Wind_y=df_resources.loc[[8]]
    Purch_y=df_resources.loc[[10]]

    # Yearly costs and monthly rates (Oct-Sep)
    costs=pd.read_excel('net_rev_data.xlsx',sheet_name=3,skiprows=[0,3,4,5], usecols=[8])*pow(10,3)
    costs_y=np.repeat(costs.values,1188)

    costs_y=costs_y

    PF_rates=pd.read_excel('net_rev_data.xlsx',sheet_name=4,skiprows=np.arange(13,31), usecols=[0,7])
    PF_rates.columns=['month','2018']
    IP_rates=pd.read_excel('net_rev_data.xlsx',sheet_name=5,skiprows=np.arange(13,31), usecols=[0,7])
    ...
    df_synth_wind=pd.read_csv('E:/Research/PhD/BPA/wind_power_sim.csv', usecols=[1])
    ...
    MidC=pd.read_csv('E:/Research/PhD/BPA/MidC_daily_prices_new.csv').iloc[:, 1:]
    MidC=MidC.iloc[:,0]
    ...
    CAISO=pd.read_csv('E:/Research/PhD/BPA/CAISO_daily_prices.csv').iloc[:, 1:]
    ...
    Used_TF= 0                #used TF over the 20 year enesemble    #total BPA resources
    ...
    for i in SD.index:
        ...
        #yearly simulation
            if ((i+1)/365).is_integer():
                ...
                if (contract=="collar") | (contract=="ind_swap"):
                    BPA_Net_rev_y.loc[year,0]=(BPA_rev_d.loc[i-364:i,0]).sum() + df_payout[year-1] + call_prem - ins_prem - call[year-1] - costs_y[year-1]
                else: BPA_Net_rev_y.loc[year,0]=(BPA_rev_d.loc[i-364:i,0]).sum() + df_payout[year-1] - ins_prem - costs_y[year-1]
                ...
                #ensembles     
                if year%20 == 0:
                    Result_ensembles_y['ensemble' + str(e)] = pd.DataFrame(data= np.stack([Reserves.loc[year-19:year,0],TTP.loc[year-19:year,'TTP'],TF.loc[year-19:year,'TF1'],TF.loc[year-19:year,'TF2'],
                                                                            CRAC_y.loc[year-19:year,0],BPA_Net_rev_y.loc[year-19:year,0]], axis=1), 
                                                                            columns=['Reserves','TTP','TF1','TF2','CRAC','Net_Rev'])
                    ...
                    Total_TF = 2.7*pow(10,9)  #Remaining borrowing authority 
